...
**Settlements** (`_makeCityTex` + `_updateSettlementVisuals`): colonies render as orbital-view city lights — a dense sodium-amber core, wandering street-light filaments with branches, a ring road, and satellite districts linked by dotted highways, drawn on an equirect texture wrapped on a thin shell that hugs the planet's curvature. The map redraws at population milestones (tier = pop/8: more arms, more districts, wider extent) and the city glow softly flickers.

**Nebulae** (`_makeNebulaTex`): domain-warped FBM clouds — wispy filamentary structure instead of round blobs, dark dust lanes carved through the bright gas, white-hot cores where the gas is densest, color drifting between two palette stops (crimson/violet, teal/cyan, ember/magenta, sea-green/blue), and embedded newborn stars with diffraction crosses. Three large clouds plus five small wisps reusing the textures at low opacity.
